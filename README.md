...
This is a clean-room redesign of the web stack, and a proof that a simple one was possible. It argues that the web's present complexity was a path taken, not the only path, and it backs that argument with a working proof of concept.

It is not a product, not a browser you should use, and not an attempt to replace the web. It is a thought experiment carried far enough to run: a demonstration that the single hardest thing about the modern web, the reason a fourth browser engine is effectively infeasible, does not have to be hard.

# Start Here

1. [The paper: What the Web Could Have Been](https://esmith.project802.io/projects/clean-room-web/what-the-web-could-have-been/). The full argument: how the web accreted, the two structural problems underneath it, and the architecture that avoids them. The [source Markdown](./paper/what-the-web-could-have-been.md) lives in this repo.
2. [The proof of concept](./poc/). The smallest thing that demonstrates the architecture is real.
...
- Paper: complete.
- Proof of concept: not yet started. Milestone ladder defined; see the [PoC README](./poc/README.md) for the build plan.
...
- One execution target. A single neutral sandboxed bytecode (WASM); no language is privileged or permanent.
...
- Spec-as-conformance-suite. The executable test suite *is* the spec, so a new engine passes a test instead of reverse-engineering an incumbent.
...
This is not a pitch, and the paper does not pretend the design is free. It treats its real limitations directly: the install-base moat that makes adoption near-impossible, the migration burden, the unsettled seam where documents meet embedded app islands, and the user-agency cost of moving layout out of the engine. See the paper's [Limitations](./paper/what-the-web-could-have-been.md#limitations) section. Naming these is part of the point: the value of the proof is not that it ships, it is that it shows an alternative was possible.
